...
class LLMService:
    ...
    def _build_client(self) -> None:
        ...
        if self._provider == "gemini":
            ...
            self._gemini = genai.Client(api_key=self._settings.api_key)
            ...
            raise ValueError(f"Unsupported LLM provider: {repr(self._provider)}")
    ...
    async def aclose(self) -> None:
        ...
        # NOTE: google-genai's client manages its own pool; nothing to await.
    ...
    async def call(self, user_prompt: str, system_prompt: str = "") -> str:
        ...
        async for attempt in llm_retry():
            with attempt:
                ...
                return await self._call_groq(user_prompt, system_prompt)
    ...
    async def call_with_tools(
        ...
        async for attempt in llm_retry():
            with attempt:
                ...
                return await self._call_groq_tools(messages, tools, system_prompt)
...
async def _call_groq_tools(
        self, messages: list[dict], tools: list[dict], system_prompt: str
    ) -> dict:
        """Groq tool calling: returns the normalised tool-call dict."""
        pass
...
def _parse_openai_response(resp: Any) -> dict:
    """Normalise an OpenAI chat response into our common dict shape."""
...
def __parse_groq_response(response: Any) -> dict:
    pass
